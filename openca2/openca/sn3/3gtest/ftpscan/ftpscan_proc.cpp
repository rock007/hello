...
// * ================================================================================ *
t_int _func_ftpprocfile_realtime(t_pcstr s_fname_in, t_pcstr s_bussopid)
{
	LOG(LOG6, "_func_ftpprocfile_realtime() file='%s', s_bussopid='%s'\n", s_fname_in, s_bussopid);
	...
	while(g_ftpscan.m_bruning)
	{
		...
		if(0 == g_ftpscan.p_buf_command->pack_pop(s_rpack, i_plen)) 
		{
			...
			else
			{
				...
				g_ftpscan.m_cbuss = s_msg.c_str()[0];
				
				i_ret = _func_ftpprocfile_realtime(s_fname.c_str(), s_bussopid.c_str());
