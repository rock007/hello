...
//public:
	...
	UDP_TABLE_STRUCT_T()
	{
		memset(&next[0], 0, sizeof(int)*TABLE_UDP_RESEND_KEY_NUMBER);
		ipre = 0;
		inext = 0;
		memset(&addrs[0], 0, sizeof(ipport_t)*MAX_DEST_ADDR_NUMBER);
		t_willDelete = 0;
		len = 0;
		buffer = NULL;
		sent = 0;
...
//public:
	...
	{
		cnt = 0;
		memset(&hash1[0], 0, sizeof(int)*HASH_TABLE_UDP_RESEND);
		memset(&hash2[0], 0, sizeof(int)*HASH_TABLE_UDP_RESEND);
		memset(mapbuf, 0, MAX_TABLE_UDP_RESEND/4);
	};
};
...
public:
	...
	int		SetTcpSockfd(int sock);
	int		SetpLog(LogFile * plog1);
	int		SetTcpFlag(int flag);
...
template<class MSG_KEY>
int UDPResendManager_t<MSG_KEY>::SetTcpSockfd(int sock) 
{
	sockfd = sock;
	return 1;
}
...
template<class MSG_KEY>
int UDPResendManager_t<MSG_KEY>::SetTcpFlag(int flag) 
{
	flag_tcp = flag;
	return 1;
}
...
int UDPResendManager_t<MSG_KEY>::RemoveResponse(MSG_KEY * keystruct, unsigned int ip, unsigned short port)
{
	...
	while ( (index = table_msg.SelectNext(key, index, 1)) )
	{
		...
		if ( plog ) plog->logwrite(4, "UDPM(%d)::RemoveResponse(): UDP(%d) delete response resend %d/%d times to '%s:%d' msg '%s' %d bytes, key='%s'\n", 
			sockfd, 
			index, 
			ptable_msg[index].sent, 
		...
		isSomeDeleted = 1;
		//TANG 20050513										//delete here
		if ( ptable_msg[index].buffer ) 
		{
			free(ptable_msg[index].buffer);
			malloc_count--;
			if ( plog ) plog->logwrite(6, "UDPM(%d)::RemoveResponse(): --malloc_count=%d\n", sockfd, malloc_count);
		}
	...
	{
		//TANG 20050513
		//don't call RunOnce(1) to delete
		//RunOnce(1);										//delete above
	}
...
int UDPResendManager_t<MSG_KEY>::SendMsgResponse(char * msgname, MSG_KEY * keystruct, void * buf, int len, unsigned int ip, unsigned short port, int t_lifetime)
{
	...
	if ( plog ) plog->logwrite(6, "UDPM(%d)::SendAnswer(): ++malloc_count=%d\n", sockfd, malloc_count);
...
int UDPResendManager_t<MSG_KEY>::SendMsgToMultiServer(char * msgname, MSG_KEY * keystruct, void * buf, int len, int server_num, ipport_t * paddr, int ntimes, int delayms)
{
	...
	//msg1.addrs[0].ip = ip;
	//msg1.addrs[0].port = port;
	for ( int j=0; j<(server_num - iserver); j++ )
	{
		msg1.addrs[j].ip   = paddr[iserver+j].ip;
		msg1.addrs[j].port = paddr[iserver+j].port;
	}
	//
	msg1.t_willDelete = 0;
	...
	if ( plog ) plog->logwrite(6, "UDPM(%d)::SendRequest(): ++malloc_count=%d\n", sockfd, malloc_count);
...
int UDPResendManager_t<MSG_KEY>::RecvMsg(MSG_KEY * keystruct, unsigned int ip, unsigned short port, int flag, int t_lifetime, void * respmsg, int respmsglen)
{
	...
	if ( flag == RECVRESP_DELETE_ALL_REQUEST )
	{
		...
		while ( (index = table_msg.SelectNext(key, index, 0)) )
		{
			...
			//TANG 20050805
			//delete here
			if ( ptable_msg[index].t_willDelete && time(NULL) >= ptable_msg[index].t_willDelete )
			{
				if ( plog ) plog->logwrite(4, "UDPM(%d)::RecvMsg(): UDP(%d) remove resend %d/%d times to %s:%d msg '%s' %d bytes\n", 
					sockfd, 
					index, 
					ptable_msg[index].sent, 
					ptable_msg[index].totalSend, 
				...
				if ( ptable_msg[index].buffer ) 
				{
					free(ptable_msg[index].buffer);
					malloc_count--;
					if ( plog ) plog->logwrite(6, "UDPM(%d)::RecvMsg(): --malloc_count=%d\n", sockfd, malloc_count);
				}
				table_msg.Delete(index);
				index = 0;
			}
...
int UDPResendManager_t<MSG_KEY>::SendTo(int udpsock, void * buf, int len, unsigned int ip, unsigned short port)
{
	...
	if ( flag_tcp ){
		//TCP
		if ( writen(udpsock, buf, len) == 0 ){
			if ( plog ) plog->logwrite(1, "UDPM::writen() error=%d\n", errno);
			return 0;
		}
	}
	else{
		//UDP
		if ( sendto(udpsock, (char *)buf, len, 0, (struct sockaddr *)&to_addr, sizeof(struct sockaddr_in) ) < 0 ){
    	...
    	if ( ptable_msg[index].t_willDelete )
    	{
    		if ( tnow >= ptable_msg[index].t_willDelete )
    		{
	    		...
	    		if ( ptable_msg[index].buffer ) 
	    		{
	    			free(ptable_msg[index].buffer);
	    			malloc_count--;
	    			if ( plog ) plog->logwrite(6, "UDPM(%d)::RunOnce(): --malloc_count=%d\n", sockfd, malloc_count);
	    		}
	    		table_msg.Delete(index);
	    		index = tmpi;
	    		continue;
...
int UDPResendManager_t<MSG_KEY>::UpdateMsgResponse(char * msgname, MSG_KEY * keystruct, void * buf, int len, unsigned int ip, unsigned short port, int t_lifetime)
{
	...
	if ( plog ) plog->logwrite(6, "UDPM(%d)::UpdateAnswer(): ++malloc_count=%d\n", sockfd, malloc_count);
...
//return 0 for error
template<class MSG_KEY>
int UDPResendManager_t<MSG_KEY>::writen(int fd, void *vptr, int n)
{
